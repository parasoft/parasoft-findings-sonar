...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:variable name="isCPPTestReport" select="/ResultsSession/@toolName = 'C++test'"/>
        ...
        <xsl:attribute name="file">
            <xsl:choose>
                <xsl:when test="$isCPPTestReport">
                    <!-- Currently, adding file attributes is not supported when the report is generated by CPPTest. -->
                </xsl:when>
                <xsl:otherwise>
